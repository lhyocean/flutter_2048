...
class BoardGridWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    double width =
        (boardSize.width - (_state.level + 1) * _state.cellPadding) /
            _state.level;
    List<CellBox> _backgroundBox = List<CellBox>();
    for (int r = 0; r < _state.level; ++r) {
      for (int c = 0; c < _state.level; ++c) {
        CellBox box = CellBox(
...
class _GameWidgetState extends State<GameWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    List<DropdownMenuItem> menus=_getMenus();
  ...
  List<DropdownMenuItem> _getMenus() {
    List<DropdownMenuItem> list=new List();

    for(int i=3;i<8;i++){
      DropdownMenuItem item=DropdownMenuItem(child: Text("level"+i.toString()),value: i,);
      list.add(item);
    }
    return list;
  }
...
class AnimatedCellWidget extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    double width = (boardSize.width - (state.level + 1) * state.cellPadding) /
        state.level;
    if (cell.number == 0) {
      return Container();
    } else {
